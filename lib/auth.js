...
async function requireAuth(req) {
  ...
  const result = await new Promise((resolve) => {
    ...
    r.on('error', (err) => { console.error('[requireAuth] network error:', err.message); resolve({ status: 500, body: {} }); });
    r.setTimeout(8000, () => { r.destroy(); console.error('[requireAuth] Supabase timeout'); resolve({ status: 500, body: {} }); });
    r.end();
  });
  if (result.status !== 200) console.error('[requireAuth] Supabase returned', result.status, JSON.stringify(result.body));
