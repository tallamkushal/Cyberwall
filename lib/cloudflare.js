...
function cfGraphQL(query, variables) {
  return new Promise((resolve, reject) => {
    ...
    const r = https.request(opts, resp => {
      let raw = '';
      resp.on('data', c => raw += c);
      resp.on('end', () => { try { resolve(JSON.parse(raw)); } catch (e) { reject(new Error('CF GraphQL parse error')); } });
    });
    r.on('error', reject);
    r.setTimeout(15000, () => { r.destroy(); reject(new Error('CF GraphQL timeout')); });
    r.write(payload);
    r.end();
  });
}

module.exports = { cfGet, cfGetZoneId, cfGraphQL, CF_EMAIL, CF_API_KEY };
