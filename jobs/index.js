...
// Render free tier sleeps after 15 min of no external requests.
// This pings the server's own /health endpoint every 10 minutes.
// NOTE: For reliable keep-alive also set up UptimeRobot → https://cyberwall.onrender.com/health
function selfPing() {
  const url = process.env.RENDER_EXTERNAL_URL || 'https://cyberwall.onrender.com';
  const target = new URL('/health', url);
  const mod = target.protocol === 'https:' ? https : http;
  const req = mod.request({ hostname: target.hostname, path: target.pathname, method: 'GET', timeout: 10000 }, res => {
    res.resume();
  ...
  try {
    ...
    for (const p of profiles) {
      ...
      createAlert(p.id, 'report', 'info',
        ...
        `This week ProCyberWall protected ${p.domain}${count > 0 ? ` and generated ${count} threat alert${count > 1 ? 's' : ''}` : ' with no major threats detected'}. Your website remained fully protected throughout the week.`
      ).catch(() => {});
    }
...
function start() {
  ...
  setInterval(sendWeeklyDigest,         24 * 60 * 60 * 1000);   // checked daily, runs Monday
  setInterval(sendMonthlyReportReminder,24 * 60 * 60 * 1000);   // checked daily, runs 1st
